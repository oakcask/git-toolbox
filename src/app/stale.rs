...
use crate::{
    config::{Configuration, ProtectedBranches},
    git::credentials::remote_callbacks,
    git::{GitTime, RemoteRef},
    reltime::Reltime,
};
...
impl Application {
    pub fn from_options(options: StaleOptions) -> Result<Self, Box<dyn Error>> {
        ...
        let now = Local::now();
        let since = options.since.map(|relative| now - relative);

        let command = if options.remote {
        ...
        match self {
            Self::DeleteUpstreamBranches { repo, visitor } => {
                let mut refspecs = visitor.for_each_branches(
                    ...
                    |mut refspecs, branch| {
                        ...
                        if let (Some(remote_name), Some(branch_name)) = (upstream, branch_name) {
                            ...
                            // Refspecs use <src>:<dst>, so an empty src deletes dst.
                ...
                for (remote_name, refspecs) in refspecs.drain() {
                    ...
                    push_refspecs(&repo, &mut remote, &remote_name, &refspecs)?;
                }
...
fn push_refspecs(
    ...
    if let Err(error) = remote.push(refspecs, Some(&mut push_options)) {
        warn!("failed to remove branches from {remote_name}: {error}");
    }
...
mod tests {
    ...
    fn create_local_branch_visitor(
        ...
        let protected_branches =
            crate::config::Configuration::new(&config).dah_protected_branches()?;
    ...
    fn create_remote_branch_visitor(
        ...
        repo.config()?
            ...
            .set_str("dah.protectedbranch", "develop:release/*")?;
        let config = repo.config()?;
        let config = crate::config::Configuration::new(&config);
    ...
    fn head_branch_is_always_ignored() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let visitor = create_local_branch_visitor(&repo)?;
        let branch = repo.find_branch("main", BranchType::Local)?;

        assert!(!visitor.match_branch(&branch)?);
    ...
    fn protected_branches_are_ignored_even_with_prefix_filters(
        ...
        let (_tmpdir, repo) = create_repo()?;
        let mut visitor = create_local_branch_visitor(&repo)?;
        visitor.branches = vec!["release/".to_owned()];
        let branch = repo.find_branch("release/v1", BranchType::Local)?;
    ...
    fn unprotected_branch_is_ignored_when_prefix_filters_do_not_match(
        ...
        let (_tmpdir, repo) = create_repo()?;
        let mut visitor = create_local_branch_visitor(&repo)?;
        visitor.branches = vec!["bugfix/".to_owned()];
        let branch = repo.find_branch("feature/old", BranchType::Local)?;
    ...
    fn for_each_branches_without_since_selects_only_branches_without_upstream(
        ...
        let selected = visitor.for_each_branches(&repo, Vec::new(), |mut names, branch| {
    ...
    fn for_each_branches_with_since_can_select_branches_with_upstream(
        ...
        track_branch(&repo, "feature/old")?;
        let mut visitor = create_local_branch_visitor(&repo)?;
        visitor.since = Some(Local::now() + Duration::days(1));
    ...
    fn remote_head_is_always_ignored() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let visitor = create_remote_branch_visitor(&repo)?;
        let remote_ref = RemoteRef::new("refs/remotes/origin/HEAD".to_owned())?;

        assert!(!visitor.match_branch(&remote_ref)?);
    ...
    fn remote_for_each_branches_lists_origin_tracking_refs(
        ...
        let selected =
            visitor.for_each_branches(&repo, Vec::new(), |mut names, remote_ref, _| {
    ...
    #[test]
    fn app_rejects_remote_delete_without_push() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[test]
    fn app_rejects_remote_mode_without_origin_remote() -> Result<(), Box<dyn std::error::Error>> {
        let tmpdir = TempDir::new()?;
